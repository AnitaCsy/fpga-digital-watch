...
module decimal_display_driver (
  ...
  generate
    for (i = 0; i < 3; i = i + 1) begin : g_digit_pair
      ...
      localparam int HexOnes = 2 * i;
      localparam int HexTens = 2 * i + 1;

      assign value = value_a[i];
      assign blank = blank_a[i];
      ...
      seven_segment #(
          ...
          .digit(ones),  // Hexadecimal digit to display
          .blank(blank),
          .segments(hex_a[HexOnes])
      );

      seven_segment #(
          .ACTIVE_LOW(1)
      ) u_tens (
          .digit(tens),  // Hexadecimal digit to display
          .blank(blank),
          .segments(hex_a[HexTens])
      );
